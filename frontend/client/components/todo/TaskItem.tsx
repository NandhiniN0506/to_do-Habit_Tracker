...
export default function TaskItem({
  ...
  const [hidden, setHidden] = useState(t.status === "Completed"); // hide immediately if completed
  const [completed, setCompleted] = useState(t.status === "Completed"); // local completed state
  ...
  const update = useMutation({
    ...
    onMutate: async (patch) => {
      ...
      if (prev) qc.setQueryData<Task[]>(["tasks"], prev.map((it) => (it.id === t.id ? { ...it, ...patch } : it)));
      return { prev };
    },
  ...
  const completeTask = useMutation({
    ...
    onMutate: async () => {
      setCompleted(true); // mark completed immediately
      setHidden(true);    // hide immediately
      await qc.cancelQueries({ queryKey: ["tasks"] });
      const prev = qc.getQueryData<Task[]>(["tasks"]);
      if (prev) qc.setQueryData<Task[]>(["tasks"], prev.filter((it) => it.id !== t.id));
      return { prev };
  ...
  const startPomodoro = (duration: number) => setSession({ id: Date.now(), start: Date.now(), duration });
  ...
  const save = () => { update.mutate({ ...local }); setEditing(false); };

  if (hidden) return null; // hide completed task immediately
  ...
  return (
    <div className="rounded-xl border bg-card p-4 shadow-sm transition hover:shadow-md">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            {!editing ? (
              <>
                <h3 className={(completed ? "text-muted-foreground " : "") + "font-medium leading-tight line-clamp-2"}>{t.task}</h3>
                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                  ...
                  {t.deadline && <span className="inline-flex items-center gap-1"><CalendarDays className="h-3.5 w-3.5" /> {t.deadline}</span>}
                  {t.recurring && <span className="inline-flex items-center gap-1">• Consistency {t.consistency_score}%</span>}
                </div>
              </>
            ) : (
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="destructive" size="icon" onClick={() => del.mutate()} aria-label="Delete"><Trash2 className="h-4 w-4" /></Button>
        </div>
      </div>
      ...
      {completed && <Separator className="mt-3" />}
    </div>
  );
}
